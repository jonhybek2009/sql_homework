...
------------------------------------------------------------------------------------------- TODO 1
select  cars.brand, cars.model, cars.price from cars; --вывод бренд машины его модель и цена
------------------------------------------------------------------------------------------- TODO 2
select count(*) as  total_cars from cars; -- вывод сколько всего машин
------------------------------------------------------------------------------------------- TODO 3
select cars.brand, cars.model, cars.price from cars where brand = 'Hyundai'; --выводит машины только марки хундай
------------------------------------------------------------------------------------------- TODO 4
select cars.brand, cars.color,  cars.model, cars.price from cars where color in ('Red', 'Blue'); --выводит только те машины у которых цвета синий и красный
------------------------------------------------------------------------------------------- TODO 5
select cars.brand,cars.model,cars.year_of_issue,cars.price from cars where year_of_issue between 2000 and 2010; --выводит те машины которые выпущены с 2000 по 2010 годов
------------------------------------------------------------------------------------------- TODO 6
select count(*) as chevrolett_count from cars where brand = 'Chevrolet'; -- сколько машин марки шевролет их количество
------------------------------------------------------------------------------------------- TODO 7
select avg(cars.year_of_issue) as year_avarage  from cars; -- средне-статистический год выпуска машин
------------------------------------------------------------------------------------------- TODO 8
select * from cars where brand in ('Audi', 'Toyota', ' Kia', 'Ford'); --выводит только машины брендов ауди тойота киа и форд
------------------------------------------------------------------------------------------- TODO 9
select * from cars where cars.model LIKE '%T'; --модели автомобилей у которых название моделей начинается на букву Т
------------------------------------------------------------------------------------------- TODO 10
select * from cars where cars.model LIKE '%e%'; --автомобили у которых есть буквы 'е'
------------------------------------------------------------------------------------------- TODO 11
select * from cars where length(brand) = 5 order by brand; -- выводит бренды которые содержат 5 символов
------------------------------------------------------------------------------------------- TODO 12
select sum(price) as price_car from cars where brand = 'Mercedes-Benz'; --суммирует все цены автомобилей мерседенс бенз
------------------------------------------------------------------------------------------- TODO 13
select cars.model,cars.price from cars where price = (select max(price) from cars) or price = (select min(price)from cars); -- самая дорогая и дещевая машина
------------------------------------------------------------------------------------------- TODO 14
select * from cars where brand not in ('Toyota'); --отобразить все машины кроме тойота
------------------------------------------------------------------------------------------- TODO 15
select * from cars order by  price desc  limit 10; --10 самых дорогихх автомрьилей
------------------------------------------------------------------------------------------- TODO 16
select * from cars order by year_of_issue desc  offset 4 limit 10; -- так и не понял,вывести машины с 2005 по 2015?
------------------------------------------------------------------------------------------- TODO 17
select *from cars where year_of_issue not between 1995 and 2005; --исключает автомобили, выпущенные в промежутке 1995 и 2005
------------------------------------------------------------------------------------------- TODO 18
--SELECT color, COUNT(*) AS car_count FROM cars GROUP BY color ORDER BY car_count DESC LIMIT 1;
select cars.color,count(*) as  cars_count from cars group by cars.color order by  cars_count desc limit 1; --выводит самый распростаненный цвет среди машин и их количество
